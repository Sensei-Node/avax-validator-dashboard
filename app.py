...
def fetch_uptime():
    try:
        ...
        for item in data.get("items", []):
            ...
            location_city = item.get("node", {}).get("location", {}).get("city", "Unknown")
            location_country = item.get("node", {}).get("location", {}).get("country", "Unknown")
            ...
            formatted_uptime = round(avg_uptime * 100, 2) if isinstance(avg_uptime, (int, float)) else "N/A"
            formatted_stake_from_self = round(int(stake_from_self) / 1_000_000_000, 2) if stake_from_self else "N/A"
            formatted_stake_from_delegations = round(int(stake_from_delegations) / 1_000_000_000, 2) if stake_from_delegations else "N/A"
